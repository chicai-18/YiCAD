...
void main()
{
    ...
    float dot_val = dot(start_pt - start_in_view.xyz, dir); // check whethe the ray start point is outside the window. (roughly)
    if(dot_val < 0.0f)  // start_in_view may inside window
    {
        start_pt = vec3(start_in_view) - dir * half_width;
        para_p1 = - half_width;
	    para_p2 = - half_width;
    }
    else    // start_in_view outside window
    {
        float len2 = length(start_pt - start_in_view.xyz);
    	para_p1 = len2;
	    para_p2 = len2;
    }
