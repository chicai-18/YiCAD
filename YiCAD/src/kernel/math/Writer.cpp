...
ZipWriter::ZipWriter(const char* FileName)
  : archive(std::make_unique<MinizipNgArchiveWriter>(std::string(FileName)))
{
}

ZipWriter::ZipWriter(std::ostream& os)
  : archive(std::make_unique<MinizipNgArchiveWriter>(os))
{
}
...
void ZipWriter::writeFiles()
{
    ...
    while (index < FileList.size()) {
        ...
        archive->putNextEntry(entry.FileName);
        OutputStream str(archive->stream());
...
void ZipWriter::putNextEntry(const char* str)
{
    archive->putNextEntry(str);
}
