...
在 v3 易用性与可扩展布局改造、兼容性审查和发布门禁全部完成前，
`YICAD_PLUGIN_ABI_MAX_VERSION` 与 `YICAD_PLUGIN_ABI_VERSION` 必须保持为
`YICAD_PLUGIN_ABI_V2`。草案布局不是发布契约，不得据此发布声称支持 v3 的插件。
...
- v3 发布前允许修改受编译开关保护的候选布局，但每次修改必须对照
  `PLUGIN_ABI_V3_PLAN.md` 中的重构前快照；发布后只允许扩展明确标记为可扩展的结构。
