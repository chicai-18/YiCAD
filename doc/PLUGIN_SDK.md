...
草案 SDK 已提供默认构造层。`EntityAttributes` 默认生成活动图层、ByLayer 线型和颜色、
标准线宽 `-1`、线型比例 `1.0`、可见以及法向量 `(0, 0, 1)`。点、线和圆可以直接使用
语义重载；SDK 在调用宿主前生成临时 POD 并连接公共属性指针：
...
`ImportContainer`。省略最后一个参数即可使用相同的标准公共属性。以上对象都只存在于
插件侧，跨 DLL 边界的仍然只有临时 C ABI POD。

尚未提供领域 builder 的输入统一使用 `makeImportData<T>()`。该工厂只接受 SDK 已知的
v3 可扩展输入类型，自动清零、设置完整 `structSize`，并写入该类型的非零协议默认值：

```cpp
auto text = yicad::plugin::makeImportData<YiCadTextDataV3>();
text.height = 2.5;
text.text = {/* 插件持有的 UTF-8 数据和字节数 */};
// text.widthFactor 已为 1.0，无需填写 structSize。
```

当前默认构造层明确设置以下非零默认值：文档全局线型比例、文字宽度比例、标注测量和
分数高度比例、MTEXT 与标注行距、填充图案比例均为 `1.0`；块引用比例和行列数均为
`1`；射线及构造线方向、MTEXT 方向使用正 X 轴；外填充环索引使用 `UINT32_MAX`。
图层和实体公共属性还会设置 ByLayer 颜色、标准线宽及各自的可见/打印默认值。工厂不会
虚构名称、文字内容、半径、资源句柄等业务必需值，调用方仍须按实体语义填写它们。

填充边和填充环连续容器分别通过 `makeHatchEdgeArrayView()` 和
`makeHatchLoopArrayView()` 生成 ABI 视图；这两个入口会同时设置 `data`、`count` 和
`byteStride`。后续领域 builder 将进一步持有字符串和数组并自动连接其余嵌套视图。
在此之前，直接连接这些视图或借用指针属于下述底层接口，不应出现在普通插件路径。
