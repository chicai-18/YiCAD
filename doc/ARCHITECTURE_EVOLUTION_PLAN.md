...
| 位置 | 问题 | 说明 |
|------|------|------|
| `ActionSelectSingle.cpp` 的 `select.single` 工厂 | `ctx.view->getCurrentAction()` 返回值在没有活动 Action 时是 `nullptr`，随即被无条件解引用（`current->getEntityType()`） | 原 switch 里就是这个写法，迁移时原样保留。候选：加一条 `DISABLED_` 回归测试锁定这个可复现的崩溃条件，留给未来专门修 bug 的会话。**已修复，见 9.2 节。** |
...
- 执行中发现、原样保留的既有缺陷：`UIDlgDimensionStyle.cpp` 用字符串形式
  ...
  7.7 节记录的 `ActionSelectSingle` 空指针解引用仍在。（两者均已修复，见 9.2 节。）
...
- `Selection::selectWindow`：两条路径。窗口盖住全部实体的包围框
  （`EntityTable::getSearchBounds`，即空间搜索树根节点的包围框）时，每个实体都会命中，
  按原来的方式顺序遍历实体表；否则由 `searchEntities(min, max, …, onlyVisible=false,
  searchSubEnts=false)` 从空间搜索树取候选。树里存的就是各顶层实体的包围盒，与原先对
  `getMin()/getMax()` 的粗过滤等价（两者都把"刚好接触"算作重叠）。两条路径对每个实体
  做同一组判断（类型、可见、未删除、包围盒粗筛、窗内或交叉），结果一致。交叉选的相交
  判断搬到文件内的 `crossesWindow()`：逐实体判断若写成包含整段交叉逻辑的 lambda，
  覆盖全图的框选实测 49 ms，对照原实现 34 ms（50 万实体；推测是这个大 lambda 有两处
  调用、未被内联，未看反汇编确认），拆出后与原实现持平。
- `EntityTable::getNearestVirtualIntersection`：新增 `SpacialSearchTree::nearest`
  （best-first 最近邻：按包围框到查询点的距离由近及远展开，包围框距离不小于当前
  最优时停止），代替对渲染容器的 `getNearestEntity`。`RTree.h` 是第三方实现，只有
  矩形范围查询，最近邻用 `SpacialSearchTree.cpp` 里的派生类实现，不改第三方文件。
  顺带：临时构造线原来每次调用都 `new` 且从不释放，改为栈对象。
- `EntityTable::searchEntities`（`searchSubEnts=false` 分支）：原来按 `DmId` 建
  `unordered_map` 去重，而 `DmId` 内部是 `std::string`，覆盖全图的框选要复制并哈希
  每个实体的 ID，50 万实体时单这一步就 800 多毫秒。改为命中实体都没有父实体时不去重，
  有父实体时按指针去重。点选（`Snapper::catchEntity` 的 `ResolveNone` 分支）与
  `ActionDrawHatch` 走同一函数，同样受益。
- `SpacialSearchTree::insert` 对已在树中的实体只更新包围框，保证上一条依赖的
  "树中每个实体只有一份"。原来重复插入会留下一条 `remove` 删不掉的残余条目（边界框
  表按 ID 只记一份），实体删除后成为悬空指针。

**与方案的偏差**

| 项 | 方案 | 实际 | 理由 |
|----|------|------|------|
| 虚拟交点的改法 | "改用 R 树候选集" | 最近邻查询 | 原实现求全图最近实体，不限距离，没有天然的候选框。用框查询要么给个半径（改变语义），要么反复扩框。best-first 在树上直接求最近，结果与全量扫描一致（下面一条除外） |
| 与全量扫描的语义差异 | 只换查询方式 | 圆心在包围盒外的圆弧不再凭圆心成为最近实体 | `DmEntity::getDistanceToPoint` 取"到曲线距离"与"到 `getCenter()` 距离"中较小者，圆心也算近点。树按包围盒剪枝，而圆弧（含多段线、块参照里的圆弧段）的圆心可能落在包围盒外，凭圆心算出的近距离因此不再被考虑。整圆、点的圆心在包围盒内，不受影响。点选早就按包围盒取候选，对这类圆心同样不可达，两者行为由此一致。另外，距离完全相等时原实现取渲染容器中靠后的实体，现取先算到的 |
| "两处改造都只是换调用方式，无需新建设施" | — | 新增 `SpacialSearchTree::nearest`/`getBounds`（`EntityTable::getSearchBounds`），改了 `searchEntities` 的去重与 `insert` 的幂等性，框选在窗口盖住全部实体时回退为顺序遍历 | 只换调用方式时，覆盖全图的框选实测比原来慢约 25 倍（见下文"中间态"），瓶颈在按字符串 ID 去重；去重改好后仍慢约 2 倍，因为几乎全部命中时树遍历比不过顺序扫描，故加回退 |

**执行中确认、未修改的既有语义**

虚拟交点只对"包围盒覆盖查询点附近"的实体生效：`Information::getIntersection(…,
onEntities=true)` 先要求两个实体的包围盒相交，而临时构造线的包围盒只是查询点到
"查询点 + 单位方向"这一小段（图纸单位，与缩放无关）。最近实体只要稍远一点，就求不出
交点，函数原样返回查询点。这看起来不是有意设计，但它属于捕捉行为，与 P10 无关，本次
未改；`test_geometry_spatial_query.cpp` 的用例按这一实际行为构造，并在文件头写明。

**性能（无头对比）**

阶段 0 的运行期基线一直未采集（`doc/BASELINE.md` §4），没有 GUI 实测的改造前数据。
下表是临时基准的结果：同一测试二进制里，把改造前的 `Selection::selectWindow` 原样
复制为对照函数，与新实现交替运行，取多次中的最小值；随机均匀分布的直线段，Release，
Windows 11 Pro 22621 / MSVC 2022，2026-09-24。基准代码未入库，正确性由入库的随机
比对用例保证。单位毫秒/次。

| 操作 | 5 万 原 | 5 万 新 | 50 万 原 | 50 万 新 |
|------|-------:|-------:|--------:|--------:|
| 虚拟交点（原：仅全量求最近实体；新：整个函数） | 5.59 | 0.016 | 57.9 | 0.022 |
| 局部窗选（200×200，约含百个实体） | 1.32 | 0.025 | 16.1 | 0.039 |
| 局部交叉选 | 1.42 | 0.076 | 16.1 | 0.097 |
| 盖住全图的窗选（顺序遍历路径） | 2.93 | 2.81 | 33.6 | 33.9 |
| 盖住全图的交叉选（顺序遍历路径） | 2.88 | 2.93 | 33.6 | 34.0 |
| 盖住约 81% 面积的窗选（走树） | 3.07 | 4.39 | 33.6 | 53.5 |
| 盖住约 81% 面积的交叉选（走树） | 4.47 | 5.63 | 38.0 | 57.9 |

改造过程中的两个中间态，供参考（50 万实体，盖住全图的窗选）：只换调用方式、仍按
字符串 ID 去重时 861 ms；去重改好、尚无顺序遍历回退时 67 ms。

**验收对照**

| 9.1 节的验收标准 | 状态 |
|------------------|------|
| 大图纸上虚拟交点捕捉耗时明显下降 | 达成（50 万实体时约 58 ms → 0.022 ms） |
| 大图纸上框选耗时明显下降 | 局部框选达成（50 万实体时 16 ms → 0.04–0.1 ms）。`doc/BASELINE.md` 第 4 步测的盖住全图的框选与原实现持平，没有下降：这时每个实体都是命中，逐实体的判断本身就是全部开销。盖住大部分但不是全部实体的窗口走树，比原实现慢约 1.5 倍（上表 81% 一行）——树遍历加一遍父实体检查的开销，在几乎全部命中时比不过顺序扫描 |

新增测试 `tests/geometry/test_geometry_spatial_query.cpp`（10 例）：窗选/交叉选/
不可见/反选、盖住全部实体时两条路径结果一致、`insert` 幂等、虚拟交点的不限半径/
精确距离剪枝/跳过不可见、圆弧圆心差异，以及一个随机用例——400 个实体上 300 次最近邻、
200 次端到端虚拟交点（其中过半真正求出交点，避免比对退化成两边都返回查询点）、100 次
窗选与交叉选（其中五分之一盖住全部实体，走顺序遍历），逐一与原全量扫描的算法比对。
